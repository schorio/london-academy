									...
									<?php
										...
										if ($_SESSION["role"] !== $config["ROLES"][0]) {
											$sql = "SELECT * FROM employee 
													...
													WHERE code_departement='$ut_departement' ORDER BY nom ASC ";
										}
										else {
											$sql = "SELECT * FROM employee
														JOIN departement ON employee.id_departement=departement.id_departement 
														...
														JOIN grade ON employee.id_grade=grade.id_grade 
														JOIN categorie ON employee.id_categorie=categorie.id_categorie 
														JOIN statut ON employee.id_statut=statut.id_statut ORDER BY nom ASC ";
										}
										...
										while($row = mysqli_fetch_array($result))
										{
											...
											$d_avancement = strtotime($row["avancement"]);
											$n_d_avancement = date('d M Y', $d_avancement);

											$d_f_contrat = strtotime($row["f_contrat"]);
											$n_d_f_contrat = date('d M Y', $d_f_contrat);

											$d_d_retraite = strtotime($row["d_retraite"]);
													...
													<tr>
														<td>
															<h2 class="table-avatar">
																<a href="/london-academy/profile.php?id='.$row["id_employee"].'" class="avatar"><img alt="image" src="/london-academy/assets/img/employee/'.$row["image"].'"></a>
																<a href="/london-academy/profile.php?id='.$row["id_employee"].' ">'.$row["nom"].'  '.$row["prenom"].'<span>'.$row["code_departement"].'</span></a>
															</h2>
														</td>														
														<td>'.$row["matricule"].'</td>
														<td>'.$row["code_categorie"].'</td>
														<td>'.$row["code_statut"].'</td>
														<td>'.$row["code_grade"].'</td>
														...
														<td class="text-right">
															<div class="dropdown dropdown-action">
																...
																<div class="dropdown-menu dropdown-menu-right">
																	<a class="dropdown-item" href="/london-academy/edit_employee.php?edit_id='.$row["id_employee"].'"><i class="fa fa-pencil m-r-5"></i> Modifier</a>
																	<a class="dropdown-item" href="#" data-toggle="modal" data-target="#delete_employee'.$row["id_employee"].'"><i class="fa fa-trash-o m-r-5"></i> Supprimer</a>
																</div>
															</div>
														</td>
													</tr>
...
<!-- <tbody>
	<tr>
		<td>
			<h2 class="table-avatar">
				<a href="profile.php?id='.$id_employee.'" class="avatar"><img alt="image" src="employees/'.$image.'"></a>
				<a href="profile.php?id='.$id_employee.' ">'.$nom.'." ".'.$prenom.'<span>'.$code_departement.'</span></a>
			</h2>
		</td>
		<td>'.$matricule.'</td>
		...
		<td class="text-right">
			<div class="dropdown dropdown-action">
				<a href="#" class="action-icon dropdown-toggle" data-toggle="dropdown" aria-expanded="false"><i class="material-icons">more_vert</i></a>
				<div class="dropdown-menu dropdown-menu-right">
					<a class="dropdown-item" href="#" data-toggle="modal" data-target="#edit_employee'.$id_employee.'"><i class="fa fa-pencil m-r-5"></i> Modifier</a>
					<a class="dropdown-item" href="#" data-toggle="modal" data-target="#delete_employee'.$id_employee.'"><i class="fa fa-trash-o m-r-5"></i> Supprimer</a>
				</div>
			</div>
		</td>
	</tr>
